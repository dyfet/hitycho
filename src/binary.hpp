...
inline auto to_string(const hitycho::byte_array& ba) {
    return ba.to_hex();
}
...
inline auto to_b64(const Binary& bin) {
    return util::encode_b64(to_byte_view(bin));
}
template <typename Binary>
inline auto to_hex(const Binary& bin) {
    return util::encode_hex(to_byte_view(bin));
}
} // namespace hitycho

namespace std {
template <>
struct hash<hitycho::byte_array> {
    auto operator()(const hitycho::byte_array& b) const noexcept {
        if (!is(b))
            return std::size_t(0);
